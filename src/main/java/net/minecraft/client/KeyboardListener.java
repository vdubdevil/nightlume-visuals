...
public class KeyboardListener
{
    ...
    public void tick()
    {
        ...
        boolean reloadDown = InputMappings.isKeyDown(
                this.mc.getMainWindow().getHandle(),
                GLFW.GLFW_KEY_F5
        );

        boolean down = InputMappings.isKeyDown(this.mc.getMainWindow().getHandle(), ClickGuiScreen.getBind());

        if (reloadDown && !this.reloadKeyDown) {
        ...
        this.reloadKeyDown = reloadDown;
        if (down && !this.clickGuiKeyDown) {
            mc.displayGuiScreen(new ClickGuiScreen());
        }

        this.clickGuiKeyDown = down;
